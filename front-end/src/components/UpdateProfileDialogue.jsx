...
const UpdateProfileDialogue = ({ open, setOpen }) => {
  ...
  // Update the input state when user data is fetched
  useEffect(() => {
    if (user) {
      setInput({
        fullname: user?.fullname || '',
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => setOpen(isOpen)}>
      <DialogContent className="sm:max-w-[425px]">
        ...
        <form onSubmit={submitHandler} action="" className="w-full max-w-4xl">
          ...
          <DialogFooter>
            {loading ? (
              ...
              <Button type="submit" className="w-3/4 items-center my-4">
                Update
              </Button>
            )}
          </DialogFooter>
